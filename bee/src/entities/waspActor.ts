...
export type WaspPhase =
  "gone" | "arriving" | "seeking" | "chasing" | "veering" | "leaving";
...
export class WaspActor {
  ...
  leave(): void {
    if (this.phase === "leaving" || this.phase === "gone") {
      return;
    }
    this.phase = "leaving";
    this.phaseTime = 0;
  ...
  update(dt: number, beePosition: THREE.Vector3): WaspEvent | null {
    ...
    switch (this.phase) {
      case "arriving":
        ...
        if (
          this.phaseTime >= WASP.arriveSeconds ||
          this.position.distanceTo(this.hive) < 12
        ) {
          this.phase = "seeking";
          this.phaseTime = 0;
        }
  ...
  private steerToward(dt: number, target: THREE.Vector3): void {
    ...
    tmpForward.y = THREE.MathUtils.clamp(
    ...
    if (tmpForward.length() > maxChange) {
      tmpForward.setLength(maxChange);
    }
