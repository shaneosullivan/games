...
```bash
npm --prefix penguin run dev         # dev server, hot reload
npm --prefix penguin run typecheck   # tsc --noEmit
npm --prefix penguin run build       # typechecks, then one self-contained dist/index.html
```
...
Always typecheck before calling a change done; `npm --prefix penguin run build`
does it for you.
