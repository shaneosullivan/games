 ...
 *
 * Form controls are exempt from the tap rules so buttons still take a tap.
 *
 * Brought over whole from the bee game, which is the point of it existing
 * there: this is a list of iOS quirks somebody had to find out one at a time,
 * and finding them out again for each game would be the worst possible use of
 * anybody's afternoon.
 */
export function lockZoom(): void {
   ...
   * has — so walk up from whatever was touched looking for a scroller with
   * room left in it, and leave that one alone.
   */
