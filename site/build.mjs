...
function renderInstallHead({title, manifest, up = ""}) {
  return `    <link rel="manifest" href="${manifest}" />
    ...
    <!--
      Which build this page is. It answers the question that costs the most
      time when something is missing from a copy somebody else is looking at:
      are they running what was pushed, or a copy a cache kept? Compare it with
      version.json, which is always fetched from the network.
    -->
    <meta name="chofter-build" content="${BUILD_ID}" />`;
}
 ...
 * already played.
 *
 * Strategy: pages come off the network with the cache as the fallback, and
 * everything else is stale-while-revalidate. Games are one big self-contained
 * HTML file each, so caching one on first play is both cheap and exactly
 * right — but serving that copy back while the new one downloads for next time
 * left every game a build behind, which is the one thing a cache must not do
 * to a page somebody has just been handed a link to.
 */
...
self.addEventListener('fetch', (event) => {
  ...
  // A page comes off the network whenever there is one, and only falls back to
  // the cache when there is not.
  //
  // This used to be stale-while-revalidate like everything else, which for a
  // page means: serve the copy from last time, fetch the new one, put it in
  // the cache for *next* time. A game opened from a cold start was therefore
  // permanently one build behind — a change pushed today showed up the day
  // after, on the second visit — and the gallery's update banner could not
  // help, because it only notices a build landing while a tab is already open.
  // Somebody was told a button existed, opened the game, and it was not there.
  //
  // A page here is one self-contained HTML file, so this is one request per
  // visit, and the cached copy still answers the moment the network does not.
    ...
    event.respondWith(
      caches.open(CACHE).then(async (cache) => {
        try {
          ...
          // Never seen: open the gallery rather than the browser's error page.
