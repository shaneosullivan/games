...
```bash
npm --prefix the-chase run dev         # dev server, hot reload
npm --prefix the-chase run typecheck   # tsc --noEmit
npm --prefix the-chase run build       # typechecks, then one self-contained dist/index.html
```
...
Always typecheck before calling a change done; `npm --prefix the-chase run build`
does it for you.
