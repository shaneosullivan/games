...
function body(a: Assembly, L: number, W: number, colour: number): void {
  ...
  // Below it, the lower body in pieces around the wheels: the sills between
  // them, the bumpers beyond them, and over each one a panel with a round
  // arch cut out of it. A hull cannot have a hole in it, and a car whose
  // wheels run through its sides — which the first go at this was — looks
  // like a toy with its wheels pushed on.
  const top = TAXI.floor + 0.3;
  const low = TAXI.clearance;
  const r = TAXI.arch;
  const frontAxle = TAXI.front * L;
  const rearAxle = TAXI.rear * L;
  ...
  for (const z of [frontAxle, rearAxle]) {
    ...
    cut.moveTo(-r, top);
    cut.lineTo(r, top);
    cut.lineTo(r, TAXI.wheel);
    cut.absarc(0, TAXI.wheel, r, 0, Math.PI, false);
    cut.closePath();
    ...
    lining.moveTo(r, TAXI.wheel);
    lining.absarc(0, TAXI.wheel, r, 0, Math.PI, false);
    lining.lineTo(-inner, TAXI.wheel);
    lining.absarc(0, TAXI.wheel, inner, Math.PI, 0, true);
    lining.closePath();
    a.add(across(lining, W * 0.9, z), "rubber", TAXI.trim);
  }
}

/** A shape drawn in the side view — along the car and up — pushed out across
 *  the car `wide`, centred, at `z` along it. */
function across(
  shape: THREE.Shape,
  wide: number,
  z: number,
