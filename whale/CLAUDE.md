...
```bash
npm --prefix whale run dev         # dev server, hot reload
npm --prefix whale run typecheck   # tsc --noEmit
npm --prefix whale run build       # typechecks, then one self-contained dist/index.html
```
...
Always typecheck before calling a change done; `npm --prefix whale run build`
does it for you.
